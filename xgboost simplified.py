...
def get_user_device_type_time(fpath='../input/sessions.csv'):
    ...
    device_type_time = pd.pivot_table(sessions, index=['user_id'], columns=['device_type'], 
                                      values='secs_elapsed', aggfunc=sum, fill_value=0)
    ...
    for column in device_columns:
        device_type_time[column+'_pct'] = device_type_time.apply(lambda row: calculate_pct(row[column],
                                                                                           row['total_elapsed_time']), axis=1)
